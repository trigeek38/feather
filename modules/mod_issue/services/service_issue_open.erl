...
get_select_statement(Id, Context) ->
    ...
    From = "from pivot_mod_issue i join rsc r on i.rsc_id = r.id left join rsc a on i.assigned_id = a.id",
...
add_stuff([H|T], NewIds) ->
   ...
   AssignedTo = z_convert:to_list(proplists:get_value(assigned_to,H)),
   Title = z_convert:to_binary(string:join([RscTitle, " - ", AssignedTo, " - ", IssueDetail, "..."], " ")),
   H2 = [{url, Url}|H],
   H3 = [{allDay, false}|H2],
   H4 = [{title, Title}|H3],
   H5 = add_now(H4),
